-- bookUser 접속
-- books Tablerhk author, company table은 Relation관계까 있다.
--      book의 bk_ccode와 company의 cp_code
--      book의 bk_acode와 company의 au_code
...
-- JOIN 데이터가 누락되거나 null 이 출력되는 것은 데이터에 문제가 발생한 것이다.
-- "참조무결성"이 무너졌다.
...
-- Relation 설정이 된 table간에 참조무결성을 보장하기 위한 제약조건을 설정할 수 있다.
-- 이 제약조건을 FOREIGN KEY 설정 이라고 한다.
-- 이 제약조건을 "FOREIGN KEY"(외래키) 설정 이라고 한다.

-- tbl_books와 tbl_company를 참조무결성 설정
-- tbl_company의 PK를 참조하여 tbl_books 데이터에 출판사 정보를 연동
-- tbl_company를 parent라고 하며 REFERENCES 테이블이라고 한다.
...
-- 데이터를 author에서 삭제하려고 하면 오류가 발생한다.
DELETE FROM tbl_author
WHERE au_code = 'A0035';
-- A0001을 삭제한 상태에서 도서 정보 추가
...
-- 데이터에 문제가 있을 경우 문제를 해결한 후 FK설정이 가능하다.
-- parent 테이블에 데이터를 추가하거나 child 데이터를 삭제하는 방법이 있다.
