...
-- PK로 설정하려고 하는 칼럼의 데이터가 PK조건(유일성, NOT NULL)을 만족하지 않는
-- 데이터가 있으면 명령이 실패한다
...
도서정보는 어플로 만들지 전에 사용하던 데이터인 관계로 데이터베이스의 규칙에
다소 어긋난 데이터가 있다.

저자 항목(칼럼)을 보면 저자가 2명 이상인 데이터가 있고 또한 역자도 2명 이상인 경우가 있다.

데이터를 저장할 칼럼을 크게 설정하여 입력(import)하는데는 문제가 없는데
저자나 역자를 기준으로 데이터를 여러가지 조건을 부여하여 조회를 하려고 하면
문제가 발생할 수 있다.
특히 저자이름으로 GROUPPING을 하여 데이터를 조회해보려고 하면 상당히 어려움을
겪을 수 있다.
*/
...
RENAME COLUMN bk_author TO bk_autor1;

-- 파일이 없는데 NOT NULL을 설정했기 때문에 오류가 발생함.
ALTER TABLE tbl_books
ADD bk_author2 NVARCHAR2(50) NOT NULL;
...
-- 2. 제약조건에 맞는 데이터를 입력한 후 제약조건을 설정한다.
ALTER TABLE tbl_books
...
이미 2개 이상의 데이터가 저장된 경우 불리하여 원자성을 갖도록 하는 것이다.

/*
도서 정보 데이터의 제 1정규화를 수행하고 보니 저자 데이터를 저장할 칼럼이
이후에 또 변경해야 하는 상황이 발생할 수 있는 이슈가 발견되었다.
...
SELECT '(' || bk_isbn, bk_autor1 FROM tbl_books
GROUP BY '(' || bk_isbn, bk_autor1

-- 두개이상의 출력된 리스트를 합하여 1개의 리스트로 보여라
...
-- 제 2정규화가 완료된 상태에서
-- 도서정보를 입력하면서 저자정보를 추가하려면 반드시 저자정보에는 ISBN과 저자명을 포함한 데이터를 추가해주면 된다.
...
-- 9791162540619 이 도서에 저자를 추가 하고 싶을때
-- 테이블의 ba_seq칼럼에는 이미 등록된 값이 아닌 새로운 숫자를 사용하여 데이터를 추가해야 한다
-- 데이터를 추가할때마다 새로운 값이 무언인지 알아야 하는 매우 불편한 상황이 만들어지고 말았다.

INSERT INTO tbl_book_author(ba_seq, ba_isbn, ba_author)
VALUES( 35, '9791162540619' , '홍길동');
-- 35로 하는 이유는 작성한 것이 34번까지 입력이 되어 있기 때문
