...
-- 확정이 되지 않은 상태에서 ROLLBACK 명령을 수행하면
--      모든 CUD(INSERT, UPDATE, DELETE)가 취소된다.
-- CUD를 수행하여 데이터에 변화가 발생하면 명령을 완료한 후에
--      반드시 확정을 지어야 한다.
--      이때 사용하는 명령이 COMMIT이다.
-- COMMIT이 실행되어야만 저장소에 데이터가 확정된다.

ROLLBACK;

-- DROP은 테이블 자체를 삭제하는 것
-- DELETE는 테이블안에 들어있는 내용을 삭제하는것
DELETE FROM tbl_score; -- tbl_score의 모든데이터를 삭제
SELECT * FROM tbl_score; -- 리스트가 없다.
ROLLBACK; -- DELETE 명령을 취소하라
-- 데이터 복구는 가능하나 같은 작업을 하는 사람들이 추가 또는 다른 작업을 하던 것
--  들이 모두 날라감
SELECT * FROM tbl_score; -- 다시 리스트가 나타났다.

-- 모든 국어 점수가 100점이 되어버리는 상황
UPDATE tbl_score SET sc_kor = 100;


-- DELETE, UPDATE 명령을 수행할때는 옵션항목인 WHERE를 반드시 포함하자
-- 또한 특별하게 필요한 경우가 아니면 where에 들어가는 칼럼은 
-- PRAMERY KEY 대상으로 하라
...
-- DELETE와 UPDATE를 수행한 후 실수를 발견했을 때
-- 즉시 ROLLBACK을 수행하면 명령을 취소할 수 있다.
-- 하지만 PRODUCTION(실무)환경에서는 DBMS의 명령을 수행하는 사용자가 혼자가 아니므로
-- ROLLBACK 명령도 매우 신중하게 사용해야 한다.


SELECT * FROM view_score;

-- veiw를 만들때 설정한 AS 값들이 칼럼이름으로 사용된다.
SELECT * FROM view_score
WHERE 총점 < 200;

-- 총점이 200 미만인 학생들 데이터를 조회하여 총점순으로 오름차순 정렬하여 보여라
SELECT * FROM view_score
WHERE 총점 < 200
-- 특정칼럼을 중심으로 정렬하라
ORDER BY 총점;

-- 총점이 200 미만인 학생들 리스트를 영어점수 기준으로 오름차순으로 정렬하고 
-- 영어점수에 같은 값이 있다면 수학점수 기준으로 오름차순 정렬하라.
-- 영어점수, 수학점수 모두 동점자가 있다면 국어점수로 오름차순 정렬하라.
...
-- AND조건문이 들어있을때 BETWEEN값을 사용하는 것이 편하다.
SELECT * FROM view_score
WHERE 총점 BETWEEN 150 AND 200;

-- 총점이 150이상 200이하인 데이터 중에 국어점수가 70이상인 데이터를 보여라
SELECT * FROM view_score
...
-- 이러한 데이터를 출력하기 위해서는 함수로 감싸지않은 칼럼들은
-- GROUP BY 에 나열해 주어야 한다.
SELECT 학번, MAX(총점)
FROM view_score
GROUP BY 학번;
...
-- 총점(총합), 평균, 최대값, 최저값, 개수를 계산하는 것

-- 전체 데이터 개수가 몇개?
SELECT COUNT(*) FROM view_score;

-- 같은 총점끼리 묶고 총점이 같은 데이터가 몇개씩 인지 보여달라
-- 총점이 같은 학생이 몇명씩인가
SELECT 총점, COUNT(*)
FROM view_score
GROUP BY 총점;
