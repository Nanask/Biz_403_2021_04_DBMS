...
/*
iolist 테이블과 Prod 테이블간에 상품으로 JOIN을 하여 NULL값이 없는것이 확인되었다.

1. iolist 테이블에 상품코드 칼럼을 추가하고 
2. prod 테이블에서 상품코드를 가져와 iolist 테이블에 저장
3. iolist 테이블과 prod 테이블간의 상품코드를 기준으로 JOIN 할 수 있도록 
    테이블 변경을 시작한다.
...
--생성된 io_pcode 칼럼에 io_pname 칼럼의 상품이름에 해당하는 코드데이터를 tbl_product에서 가져와 저장을 해야한다.

-- 테이블의 데이터를 변경하기 위한 DML
-- tbl_iolist 전체를 반복하면서 io_pcode 칼럼에 값을 갱신하라
-- 이때 tbl_iolist의 상품으로 tbl_product 데이터를 조회하여
--      일치하는 데이터가 있으면 그 중에 상품코드 칼럼의 값을 가져와서
        io_pcode 칼럼에 저장하라

--상품명과 일치한 상품코드를 자동입력해주는 것?
...
/*
iolist 전체 데이터를 보여달라
iolist 데이터에서 상품이름을 product 테이블에서 조회하여
일치하는 상품이 있으면 리스트를 보일때 같이 보여달라.
라는 Sub Query
JOIN을 하지 않고 Sub Query를 사용하는 방식
밑에는 화면에 보여주는 용도
위에는 업데이트를 위한 코드
*/
...
-- 1. iolist로부터 거래처명, 대표자명 칼럼을 기준으로 중복되지 않은 데이터를 조회
-- 거래처명, 대표자명 순으로 정렬
...
-- 거래처정보 테이블에서 거래처 코드를 조회하여 tbl_iolist의 io_dcode 칼럼에 업데이트
UPDATE tbl_iolist
SET io_dcode = (
    SELECT d_code
    FROM tbl_dept
