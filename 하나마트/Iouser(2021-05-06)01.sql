...
-- 농사를 지어서 쌀 20kg을 생산하여 판매를 하면 5만원 정도의 금액에 판매하게 된다.
-- 쌀을 공장에서 가공하여 생산품(공산품)으로 만들게 되면 실제 20KG의 쌀을 직접
-- 판매하는 것보다 더 비싼 가격에 판매하게 된다.
-- 이때 실제 쌀보다 더 많은 이익이 발생하게 되므로 가치가 부가 되었다라고 표현한다
...
-- 전체 상품리스트와 매입, 매이출단가를 조회하기
-- 같은 상품이라도 거래 시기에 따라 매입과 매출금액이 달라질 수 있기 때문에
-- 단가 데이터중에 제일 높은 단가를 가져오기
SELECT io_pname,
...
4. 매입단가가 0인 데이터는 매출데이터에서 임의로 생성하기
    ...
    E2 항목의 값을 0.8을 곱하여 80% 가격이 되고 다시 그 금액을 1.1로 나누면 부가세를 제외한 가격이 된다.
    =ROUND(IF(C2 = 0,((E2 * 0.8)/1.1),C2),0)
    
5. 매출단가가 0인 데이터는 매입데이터에서 임의로 생성하기 
    매입단가의 20%를 추가하고 부가세 10%를 추가
    매입단가 + 20% + 10%
   F2=IF(E2=0,(D2 * 1.2) * 1.1,E2)  G2=INT(F2/10)*10
*/
...
-- 상품정보 테이블
-- DEFAULT 속성
-- INSERT를 수행할 때 값이 지정되지 않으면 자동으로 추가 될 데이터
-- 자동으로 NOT NULL로 설정된다.
...
/*
매입매출 데이터로부터 상품정보, 거래처정보 데이터를 생성하고
테이블을 생성하여 데이터를 import 했다.

매입매출데이터와, 상품정보, 거래처정보를 JOIN하기 위해서는
매입매출데이터에 상품코드, 거래처코드가 있어야한다.
그러나 현재 데이터를 코드 칼럼이 없이 이름 칼럼만 있는 상태이다.

매입매출 데이터에 상품코드, 거래처코드 칼럼을 추가하고 세 table을 JION할 수 있도록 변경하기
